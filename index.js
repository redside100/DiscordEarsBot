...
function process_commands_query(txt, mapKey, user) {
    if (txt && txt.length) {
        ...
        txt = txt.replace(/[^a-zA-Z\d\s:]/g, "").toLowerCase();

        let val = guildMap.get(mapKey);

        const im_regex = /\bim\b ([^\s]+)/
        const i_am_regex = /\bi am\b ([^\s]+)/
        const hi_regex = /\bhi\b ([^\s]+)/
        ...
        if (hi_regex.test(txt)) {
            const word_list = txt.split(" ");
            const match = word_list[word_list.indexOf("hi") + 1]
            Object.keys(userHistoryMap).forEach((user_id) => {
                if (userHistoryMap[user_id][0] === match && user_id !== user.id.toString()) {
                    // 15 secs
                    if ((Date.now() - userHistoryMap[user_id][1]) <= 15000) {
                        const embed_info = new Discord.MessageEmbed()
                            .setColor('#ff0000')
                            .setTitle(':warning:  Alert')
                            .setDescription(`<@${user.id}> did the thing.\nThey were disconnected from the voice channel :eyes:`)
                            .addField('Matching', match, true)
                            .setTimestamp()

                        const member = val.voice_Channel.guild.member(user);
                        member.voice.kick();

                        val.text_Channel.send(`<@${user.id}>`);
                        val.text_Channel.send(embed_info);
                        userHistoryMap[user_id] = [null, 0];
                    }
                }
            })
        }
